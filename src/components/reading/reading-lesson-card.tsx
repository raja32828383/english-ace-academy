...
import type { ReadingProgressRow } from "@/lib/reading";
...
interface Props {
  ...
  bookmarked: boolean;
  onToggleBookmark: (lesson: ReadingLesson, next: boolean) => void;
}

export function ReadingLessonCard({
  lesson,
  progress,
  bookmarked,
  onToggleBookmark,
  ...
  return (
    <Card className="group relative flex h-full flex-col p-4 transition-transform hover:-translate-y-0.5">
      <button
        ...
        aria-label={
          bookmarked
            ? `Remove ${lesson.title} from bookmarks`
